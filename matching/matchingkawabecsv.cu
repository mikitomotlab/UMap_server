...
int main()
{
	...
	while (1) {
		...
		for (int ii = 0; ii < db_size; ii++) {
			string inimgnum = dbfiles[ii].substr(5, 4);
			...
			ofstream outputfile("C:\\Users\\tmem\\Desktop\\kawabe\\2019\\mathing_result\\pix_count_csv\\" + infile + ",result\\locate,result1.csv", ios::app);
			outputfile << inimgnum << "," << x_str << "," << y_str << "," << th_str << "," << fixed << setprecision(4) << all_score_out << "," << endl;
			outputfile.close();
		}
