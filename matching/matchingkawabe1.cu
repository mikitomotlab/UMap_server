...
void launchMyKernel(cuda::GpuMat d_in, cuda::GpuMat *d_dbV, cuda::GpuMat *d_lcV, const int in_pixel, int *db_nzp_V, MResult *mr) {
	...
	unsigned int *d_lc_pix;
	unsigned int *d_db_pix;
	unsigned int *d_lc_con;//濃度concentration
...
int main()
{
	...
	while (KKD > zurasi * ka) {
		...
		double zurasi1 = round(PPD*zurasi*ka);
		int zurasi2 = src.cols - zurasi1;

		int n = 1;
		for (int i1 = 0; i1 < src.cols; i1++) {
			for (int j1 = 0; j1 < src.rows; j1++) {
				int x1 = i1 + zurasi1, y1 = j1;
				if (x1 < src.cols) {
					int intensity = src.at<unsigned char>(y1, x1);
					ksrc.at<unsigned char>(y1, i1) = unsigned char(intensity);
				}
				else {
					int intensity = src.at<unsigned char>(y1, x1 - zurasi1 - zurasi2);
					ksrc.at<unsigned char>(y1, i1) = unsigned char(intensity);
				}
			}
		}
		// resize
		double ratio = dbV[0].cols / (double)ksrc.cols;
		Mat rsrc = Mat::ones(ksrc.rows * ratio, ksrc.cols * ratio, CV_8UC1);
		resize(ksrc, rsrc, rsrc.size(), INTER_LINEAR);
		rsrc = ~rsrc;
